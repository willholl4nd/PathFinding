...
public class Dijkstras implements PathFind {
    ...
    public void findPath(Display.Node[][] grid, Display.Node start, Display.Node end) {
        ...
        while (openList.size() != 0) {
            ...
            current = openList.stream().min(Comparator.comparingDouble(o -> o.g)).get();
    ...
    private void run(ArrayList<Display.Node> openList, ArrayList<Display.Node> closeList) {
        openList.forEach(node -> drawVisited(node, true));
        closeList.forEach(node -> drawVisited(node, false));
    }
    ...
    private void drawVisited(Display.Node node, boolean isOpenNode) {
        if (isOpenNode) {
